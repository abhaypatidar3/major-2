...
const SEARCH_CATEGORY_ID = "nct-12102921"; // Women Gynecologist & Obstetrician ID
  ...
  try {
    ...
    return resultsObj.data.map((row) => {
      ...
      return {
        ...
        hospital: (item.compName || item.NewAddress || "Not specified")
          .replace(/<[^>]+>/g, "")
          .trim(),
        rating: item.compRating ? parseFloat(item.compRating) : null,
        experience:
          item.attr_data?.node3?.[0]?.replace(/<[^>]+>/g, "") ||
          "Not specified",
...
export const getJustdialGynecologists = catchAsyncErrors(
  async (req, res, next) => {
    ...
    try {
      ...
      const searchUrl = `https://www.justdial.com/${encodeURIComponent(city)}/Women-Gynecologist-Obstetrician-Doctors/${SEARCH_CATEGORY_ID}`;
      ...
      if (doctors.length === 0) {
        ...
        return next(
          new ErrorHandler(
            "Justdial structure changed or bot detected. Check console logs.",
            502,
          ),
        );
      }
      ...
      const message =
        status === 403
          ? "IP Blocked by Justdial. Use a VPN or Proxy."
          : error.message;
